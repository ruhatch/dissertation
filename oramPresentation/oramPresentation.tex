...
\begin{frame}
    ...
    \begin{itemize}
        \item Take in parameters $a$, the address, $op$, the operation, and $data^\ast$, the new data for a write
        \item Then have the following steps:
            \begin{itemize}
                \item Lookup position of $a$ in position map, $x$
                \item Remap $a$ to a random position
                \item Read the $x$-th path into the stash
                \item If $op$ is write, then overwrite data for $a$ with $data^\ast$ in the stash
                \item Write blocks from the stash back into $x$-th path
                \item If $op$ is a read, then return data
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{Introduction to MirageOS}
    \begin{itemize}
