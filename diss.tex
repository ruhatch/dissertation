...
\mytodo{Talk about optimising the block size both above and below the encryption layer and for various block sizes}
\mytodo{Give potential explanation for the resulting optimal parameter values}

\subsubsection{Bucket Size}

\mytodo{Talk about optimising the bucket size for various block sizes}
\mytodo{Give potential explanation for the resulting optimal parameter values and compare with the Path ORAM paper}

\subsection{Comparison with Literature}
\label{subsec:comparisonWithLiterature}

\mytodo{Talk about the failure of the initial control experiment, due to writes and reads being of different costs, and the resulting effect on the largest blockDevices}

\mytodo{Talk about a redesigned control that uses equal numbers of read and write for every block device while still using a large enough number of blocks}

\mytodo{Talk about how with more time we would run a control that performed 2 * maximumSectorSize reads and writes, so that they all on average do half reads and half writes}

\mytodo{Talk about the time experiment and the results of it compared to the control experiment}

\mytodo{Talk about the testing the overhead required or statlessness}

\mytodo{Compare the overhead required with recursive ORAM}

\mytodo{Compare the speed of recursive ORAM and discuss the resulting trade-off between speed and space and the possibility for statelessness}

\mytodo{Discuss further evaluation that would be carried out if there were more time (bandwidth overheads and such like)}
